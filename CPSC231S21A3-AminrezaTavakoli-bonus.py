...
list_x = []
list_y = []
def calc_max_list(a_list):
    """
    :param a_list: a two dimensional list
    :return: it returns the highest value
    """
    a_list.sort()
    max_list = a_list[-1]
    return max_list
...
def calc_min_list(a_list):
    """
    :param a_list: a two dimensional list
    :return: it returns the lowest value
    """
    a_list.sort()
    min_list = a_list[0]
    return min_list
...
def draw_cons(pointer, full_con_star_list_c, dict_star_name, color):
    ...
    for i in range(len(full_con_star_list_c)):
        ...
        list_x.append(screen_x_1)
        list_x.append(screen_x_2)
        list_y.append(screen_y_1)
        list_y.append(screen_y_2)
    min_x = calc_min_list(list_x)
    max_x = calc_max_list(list_x)
    min_y = calc_min_list(list_y)
    max_y = calc_max_list(list_y)

    return min_x, max_x, min_y, max_y


def draw_box(pointer, min_x, max_x, min_y, max_y, constellation_name):
    pointer.color("orange")
    pointer.penup()
    pointer.goto(min_x, min_y)
    pointer.pendown()
    pointer.goto(min_x, max_y)
    pointer.goto(min_x/2, max_y)
    pointer.penup
    pointer.goto(min_x/2, max_y + NAME_OFFSET)
    pointer.pendown
    pointer.write(constellation_name, font=("Arial", 5, "normal"))
    pointer.penup
    pointer.goto(min_x/2, max_y)
    pointer.pendown
...
def main():
    ...
    while True:
        ...
        min_x, max_x, min_y, max_y = draw_cons(pointer, full_con_star_list_c, dict_star_name, color)
        draw_box(pointer, min_x, max_x, min_y, max_y, constellation_name)
        quantifier += 1
